...
function set_valid_word_list(english_word_list,tagalog_word_list) {

    valid_word_list = english_word_list.concat(tagalog_word_list);
    console.log("valid word lists")
}
...
function score(x, y) { //where x = word and y = element
...
function match_score(x, y) { // find the smallest number
    var length = Math.min(x.length, y.length);
    if (length <= 0) return 0.0;

    var total = 0;
    for (var i = 0; i < length; i++) { //checking if the char matches
        if (x.charAt(i) == y.charAt(i)) total++;
    }

    var diff = length - total; //check if the minimum length of  matches the total similar characters
    return Math.max(1.0 - diff / 5, 0);  // return the score of similarity of characters on the corpus
}
